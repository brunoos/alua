...
function _alua.daemon.app.verify_proc(context, appname, reply)
	local app = _alua.daemon.app.apptable[appname]
...
function _alua.daemon.app.query(sock, context, argument, reply)
	...
	local app = _alua.daemon.app.apptable[name]

	if not app then
		return reply({ name = name, status = "ok" })
	end

	if not app.cache then -- keep a cache of processes and daemons
		local processes, daemons = {}, {}

		for i in pairs(app.processes) do
			table.insert(processes, i)
		end
...
function _alua.daemon.app.start(sock, context, argument, reply)
	...
	_alua.daemon.app.apptable[name] = app
...
function _alua.daemon.app.leave(sock, context, argument, reply)
	...
	if not app then
		return -- process not in application
	end

	app.processes[context.id] = nil
	context.apptable[name] = nil
