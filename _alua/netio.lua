...
function _alua.netio.request(sock, context, incoming)
	...
	end; local handler = context.command_table[request.name]
...
function _alua.netio.reply(sock, context, incoming)
	...
	context.reqtable[reply.id] = nil -- remove from sequence table
...
function _alua.netio.async(sock, cmd, arg, callback)
	...
	context.reqcount = context.reqcount or 0 -- arrange sequence record
	context.reqtable = context.reqtable or {} -- arrange sequence table
	local req = { name = cmd, id = context.reqcount } -- tag the request
