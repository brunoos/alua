...
function _alua.utils.dump(obj)
	if type(obj) == "table" then -- recursively dump tables
		local i, v = next(obj); if not i then return "{}" end
		local buf = "{ "; while i do
			-- If the element index is a string, print it.
			if type(i) == "string" then
				buf = buf .. [[["]] .. i .. [["] = ]] end
			buf = buf .. _alua.utils.dump(v); i, v = next(obj, i)
			-- If there's a next object, comma-separate it.
			if i then buf = buf .. ", " end
		end; return buf .. " }"
	end; if type(obj) == "string" then return string.format("%q", obj) end
	return tostring(obj) -- Numbers, booleans, userdata (unlikely), etc
end
